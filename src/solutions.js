...
const params=new URLSearchParams(location.search);const viewer=params.get('viewer');const viewerQuery=viewer?'?viewer='+encodeURIComponent(viewer):'';
...
function renderBrief(key){selected=routes[key];document.querySelectorAll('[data-appointment]').forEach(button=>button.classList.toggle('selected',button.dataset.appointment===key));brief.querySelector('small').textContent=selected.code;brief.querySelector('h3').textContent=selected.title;brief.querySelector('p').textContent=selected.body;agenda.innerHTML=selected.agenda.map(item=>`<li>${item}</li>`).join('');copyBrief.disabled=false;printBrief.disabled=false;brief.scrollIntoView({behavior:'smooth',block:'nearest'})}
document.getElementById('appointmentOptions').addEventListener('click',event=>{const button=event.target.closest('[data-appointment]');if(button)renderBrief(button.dataset.appointment)});
...
const initial=params.get('path');if(routes[initial])renderBrief(initial);
