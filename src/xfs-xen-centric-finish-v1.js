const XFS_RELEASE=Object.freeze({studio:'XFS',version:'1.2.0',xenCentric:true,institutionalMemory:true,proofLinks:true,manualReturn:true,contentPlane:true,solutionsGateway:true,commercialReview:true,controllerOwner:'src/ed-premiere-clean-v1.js',finaleNarrationOwner:'branch-controller'});window.XFS_RELEASE=XFS_RELEASE;document.documentElement.dataset.xfs='1.2.0';
...
const viewer=new URLSearchParams(location.search).get('viewer');const viewerParam=viewer?'&viewer='+encodeURIComponent(viewer):'';const solutionsLink=document.createElement('a');solutionsLink.className='solutions-gateway-link';solutionsLink.href='solutions.html?source=premiere'+viewerParam+'#paths';solutionsLink.textContent='Explore licensing, deployment, and Living Company paths →';reviewInvitation.append(solutionsLink);
