...
const pitchManifestPromise=fetch('assets/narration/xcm-v1/manifest.json?v=xep-001',{cache:'no-store'}).then(response=>{if(!response.ok)throw new Error('pitch-manifest-http-'+response.status);return response.json()}).then(manifest=>{if(manifest.schema!=='xen-executive-pitch/v1'||manifest.pitchId!=='ed-executive-pitch-v1'||manifest.voice!=='marin'||manifest.voiceContract!=='XVS-001-MARIN-EXCLUSIVE-v1'||manifest.approvedAudition!=='assets/narration/xen-voice-audition-v2.mp3'||!/^[0-9a-f]{64}$/i.test(manifest.sha256||'')||!manifest.path?.startsWith('assets/narration/xcm-v1/'))throw new Error('pitch-manifest-contract');pitchAudio.src=`${manifest.path}?v=${manifest.sha256}`;pitchReady=true;document.body.dataset.pitchVoice='verified';document.querySelector('.xcm-launch')?.removeAttribute('disabled');return true}).catch(error=>{document.body.dataset.pitchVoice='blocked';document.body.dataset.pitchError=String(error?.message||'manifest');document.querySelector('.xcm-launch')?.setAttribute('disabled','');return false});
