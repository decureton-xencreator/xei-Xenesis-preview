...
const pitchSlides=[
  ...
  {cue:.44,kicker:'THE ENGINE LANDSCAPE',title:'Powerful systems. Different strengths.',body:'Xen does not need to pretend the frontier platforms are weak. She shows where each engine is strongest—and why the missing layer matters.',visual:'<div class="xcm-compare"><article><b>OPENAI · CHATGPT</b><span>Frontier general intelligence · agents · tools</span></article><article><b>CLAUDE</b><span>Reasoning · writing · software work</span></article><article><b>GEMINI</b><span>Google ecosystem · multimodal context</span></article><article><b>GROK</b><span>Real-time X awareness · frontier model</span></article><article><b>PERPLEXITY</b><span>Live research · cited discovery</span></article><article class="xen-layer"><b>XEN</b><span>Identity · memory · authority · continuity · orchestration</span></article></div>'},
  {cue:.62,kicker:'THE XEN DIFFERENCE',title:'Engines answer. Xen maintains the journey.',body:'Xen is the governed architecture around intelligence: designed to understand, organize, authorize, execute, verify, and evolve.',visual:'<div class="xcm-figure"><span>GOVERNED INTELLIGENCE</span><strong>XEN</strong><div class="xcm-route"><i>UNDERSTAND</i><i>ORGANIZE</i><i>AUTHORIZE</i><i>EXECUTE</i><i>VERIFY</i><i>EVOLVE</i></div></div>'},
  {cue:.75,kicker:'THE ENVIRONMENTS',title:'One intelligence. Seven living expressions.',body:'Each environment is a working surface of the same governed intelligence—not a disconnected product menu.',visual:'<div class="xcm-figure"><span>ONE XEN · SEVEN ENVIRONMENTS</span><div class="xcm-chip-grid"><span>Living Companies</span><span>The Globe</span><span>Daily Bread</span><span>Xen Academy</span><span>Executive GPS</span><span>Memory + Truth</span><span>Media Intelligence</span></div></div>'},
...
const pitchSlideElements=[...shell.querySelectorAll('[data-pitch-slide]')];const pitchProgress=[...shell.querySelectorAll('.xcm-pitch-progress i')];const pitchBack=shell.querySelector('#xcmPitchBack');const pitchPause=shell.querySelector('#xcmPitchPause');let pitchSlideIndex=0;function showPitchSlide(next){pitchSlideIndex=Math.max(0,Math.min(pitchSlideElements.length-1,next));pitchSlideElements.forEach((slide,i)=>slide.classList.toggle('active',i===pitchSlideIndex));pitchProgress.forEach((dot,i)=>dot.classList.toggle('active',i===pitchSlideIndex));document.body.dataset.pitchSlide=String(pitchSlideIndex+1)}
...
function openConversation(){
  ...
  pitchAudio.currentTime=0;pitchAudio.muted=false;pitchAudio.volume=1;showPitchSlide(0);pitchAudio.ontimeupdate=()=>{if(!pitchAudio.duration)return;const progress=pitchAudio.currentTime/pitchAudio.duration;let next=0;for(let i=0;i<pitchCues.length;i++)if(progress>=pitchCues[i])next=i;showPitchSlide(next)};pitchAudio.onended=finishPitch;pitchAudio.onerror=()=>{document.body.dataset.warden='blocked';document.body.dataset.pitchVoice='blocked';closeConversation()};
...
pitchBack.addEventListener('click',()=>{if(!pitchAudio.duration)return;const previous=Math.max(0,pitchSlideIndex-1);pitchAudio.currentTime=pitchCues[previous]*pitchAudio.duration;showPitchSlide(previous)});pitchPause.addEventListener('click',()=>{if(pitchAudio.paused){pitchAudio.play();pitchPause.innerHTML='Ⅱ <span>PAUSE</span>';pitchPause.setAttribute('aria-label','Pause Xen pitch')}else{pitchAudio.pause();pitchPause.innerHTML='▶ <span>RESUME</span>';pitchPause.setAttribute('aria-label','Resume Xen pitch')}});
