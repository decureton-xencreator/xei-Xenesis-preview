...
for(const term of ['phoneInteractionStage','Your turn, Ed','bottom:calc(104px','body.phone-interaction-open .captions','min-height:58px'])if(!stageCss.includes(term))throw new Error(`Visible interaction stage style missing: ${term}`);
for(const term of ['interactionMap','memory:[\'.choice-row\']','awaiting-ed','replaceWith(placeholder)','visualViewport','restore()'])if(!stageJs.includes(term))throw new Error(`Visible interaction stage runtime missing: ${term}`);
